'use client';
import React, { useState, useEffect } from 'react'; import { motion } from 'framer-motion'; import { Radar, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, ResponsiveContainer, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, Legend, PieChart, Pie, Cell } from 'recharts'; import { Activity, TrendingUp, AlertTriangle, CheckCircle, FileText, Users, Building2, Anchor, Target } from 'lucide-react'; import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'; import { useLanguage } from '@/context/LanguageContext'; import { useCompany } from '@/context/CompanyContext'; import { useYear } from '@/context/YearContext'; import { INDICATORS, AXES } from '@/data/indicators';

// --- Custom Components --- const RadarCustomTick = ({ payload, x, y, cx, cy, ...rest }: any) => { return ( <text {...rest} y={y + (y - cy) / 10} x={x + (x - cx) / 10} fill="#9CA3AF" fontSize="11px" textAnchor="middle" > {payload.value} </text> ); };

export default function Dashboard() { const { t, language } = useLanguage(); const { getSelectedZone, selectedZoneId } = useCompany(); const { selectedYear } = useYear(); const selectedZone = getSelectedZone();

// --- State --- const [radarData, setRadarData] = useState<any[]>([]); const [maturityTrend, setMaturityTrend] = useState<any[]>([]); const [improvementData, setImprovementData] = useState<any[]>([]); const [riskMatrix, setRiskMatrix] = useState<any[]>([]); const [stats, setStats] = useState({ maturity: 0, compliance: 0, risks: 0, exports: 0, investment: 0, jobs: 0, omanization: 0 });

useEffect(() => { // 1. Prepare Radar Data (Dual Layer) // (Simulating calculation from INDICATORS) const processedRadar = AXES.map(axis => ({ subject: language === 'ar' ? axis.title_ar : axis.title_en, score: (Math.random() * 2) + 3, // Mock Score for Zone (3 to 5) sectorAverage: 3.5, // Mock Sector Average fullMark: 5 })); setRadarData(processedRadar);

// 2. Prepare Maturity Trend (Dual Layer Area Chart)
const trendData = [
...
// 3. Prepare Improvement Plan Data
setImprovementData([
...
// 4. Stats & Mock Data
if (selectedZone) {
    // Use zone specific data if available or fallbacks
    setStats({
        maturity: 3.3,
        compliance: 78,
        risks: 1, // Critical Risks
        exports: (selectedZone as any).exportsValue || 1200000000,
        investment: (selectedZone as any).cumulativeInvestment || 450000000,
...
// Helper for currency const formatCurrency = (val: number) => { return (val / 1000000).toFixed(1) + (language === 'ar' ? ' مليار' : 'B'); // Simplified for Demo };

return ( <div className="p-6 space-y-6 text-white" dir={language === 'ar' ? 'rtl' : 'ltr'}>
    ...
    <Card className="bg-slate-900 border-slate-800">
      <CardContent className="p-6 flex items-center justify-between">
        <div>
          ...
          <div className="text-3xl font-bold text-gold-400 mt-1">{stats.maturity} <span className="text-sm text-slate-500">/ 5</span></div>
        </div>
    ...
    <Card className="bg-slate-900 border-slate-800">
      <CardContent className="p-6 flex items-center justify-between">
        <div>
          ...
          <div className="text-2xl font-bold text-emerald-400 mt-1">{stats.investment.toLocaleString()} <span className="text-xs">OMR</span></div>
        </div>
    ...
    <Card className="bg-slate-900 border-slate-800">
      <CardContent className="p-6 flex items-center justify-between">
        <div>
          ...
          <div className="text-2xl font-bold text-blue-400 mt-1">{formatCurrency(stats.exports)} <span className="text-xs">OMR</span></div>
        </div>
          ...
          <div className="flex-1 flex flex-col">
              <div className="flex-1 grid grid-rows-5 grid-cols-5 gap-1 p-2">
                ...
                {Array.from({ length: 25 }).map((_, i) => {
                  ...
                  const hasRisk = (row === 1 && col === 3) ? 1 : (row === 0 && col === 4) ? 2 : null;
                  return (
                    <div key={i} className={`rounded ${bgClass} flex items-center justify-center relative border border-white/5 hover:border-white/20 transition-all`}>
                      {hasRisk && (
